...
class ValidateLinux(Validator):
    ...
    def check_paths(self):
        ...
        if self.db.get('storm_middle_coordinates', False):
            print(
            ...
            with Listener(on_click=self.set_storm_middle_coordinates) as y:
                y.join()
                time.sleep(1)
...
class ValidateWindows(Validator):
    ...
    def check_click_coordinates(self):
        ...
        if self.db.get('storm_coordinates', False):
            print(
            ...
            with Listener(on_click=set_storm_coordinates) as x:
                x.join()
                time.sleep(1)
        ...
        if self.db.get('storm_middle_coordinates', False):
            ...
            print('Please click in middle of PhpStorm')
            with Listener(on_click=set_storm_middle_coordinates) as y:
                y.join()
                time.sleep(1)
            print('PhpStorm middle coordinates saved successfully \U0001F44D')

        if self.db.get('tracker_app_coordinates', False):
            print(
